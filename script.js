function OlimpicCountdown() {

const olimpicGames = new Date("July 26, 2024 00:00");
const now = new Date();
const diff = olimpicGames - now;
...
const displayYear = Math.floor(diff / msInYear);
document.querySelector(".years").textContent = displayYear;
const displayDays = Math.floor((diff%msInYear) / msInDay);
...
if (diff <= 0){
    document.querySelector(".years").textContent = 0;
...
let timerID = setInterval(OlimpicCountdown, 1000);


function pictureVanish(){
    const picture = document.querySelector("img");
    picture.classList.add("vanish");
    const can = document.querySelector("#can");
    can.classList.add("canv")
}
function firework(){
